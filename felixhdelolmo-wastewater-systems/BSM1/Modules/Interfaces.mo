...
air between blower and nitrification tank."),
  ...
  partial model ASM1base "Base class of WWTP modelling by ASM1"
    ...
    WWFlowAsm1in In annotation (Placement(transformation(extent={{
              -110,-10},{-90,10}}, rotation=0)));
    WWFlowAsm1out Out annotation (Placement(transformation(extent={{
              90,-10},{110,10}}, rotation=0)));
    WWFlowAsm1out MeasurePort annotation (Placement(transformation(
  ...
  equation
    ...
    // Process Rates //
    //***** The three values "0.0000000000000000000000000000000000000000000001" were added to avoid division between 0 (it is an error that makes simulation errors)*****//
    ...
    p5 = b_a*Xba;
    p6 = k_a*Snd*Xbh;
    p7 = k_h*((Xs/(Xbh+0.0000000000000000000000000000000000000000000001))/(K_x + (Xs/(Xbh+0.0000000000000000000000000000000000000000000001))))*((So/(K_oh + So)) + ny_h*(K_oh/(K_oh
       + So))*(Sno/(K_no + Sno)))*Xbh;
    p8 = p7*Xnd/(Xs+0.0000000000000000000000000000000000000000000001);
...
"));
  ...
  connector AirFlowIn "Airflow connector"
    package WWU = WasteWater.WasteWaterUnits;
    flow WWU.VolumeFlowRate Q_air;
    annotation(
      Documentation(info = "The Airflow connector consits of a flow variable describing the exchange of
air between blower and nitrification tank."),
  ...
  connector AirFlowOut "Airflow connector"
    package WWU = WasteWater.WasteWaterUnits;
    flow WWU.VolumeFlowRate Q_air;
    annotation(
      Documentation(info = "The Airflow connector consits of a flow variable describing the exchange of
